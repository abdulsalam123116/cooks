...
! function(e) {
    ...
    i.prototype = {
        ...
        theme: function(e) {
            return void 0 === e ? this.options.theme : (console.log(this.$elementjFilestyle.attr("class").replace(/.*(jfilestyle-theme-.*).*/, "$1")), this.$elementjFilestyle.removeClass(this.$elementjFilestyle.attr("class").replace(/.*(jfilestyle-theme-.*).*/, "$1")), this.options.theme = e, this.$elementjFilestyle.addClass("jfilestyle-theme-" + this.options.theme), void 0)
        },
        ...
        constructor: function() {
            var i = this,
                n = "",
                l = i.$element.attr("id");
            "" !== l && l || (l = "jfilestyle-" + t, i.$element.attr({
                id: l
            }), t++), n = '<span class="focus-jfilestyle"><label for="' + l + '" ' + (i.options.disabled ? 'disabled="true"' : "") + "><span>" + i.options.text + "</span></label></span>", i.options.buttonBefore === !0 ? n += i.htmlInput() : n = i.htmlInput() + n, i.$elementjFilestyle = e('<div class="jfilestyle ' + (i.options.input ? "jfilestyle-corner" : "") + " " + (this.options.buttonBefore ? " jfilestyle-buttonbefore" : "") + " " + (i.options.theme ? "jfilestyle-theme-" + i.options.theme : "") + '"><div name="filedrag"></div>' + n + "</div>"), i.$elementjFilestyle.find(".focus-jfilestyle").attr("tabindex", "0").keypress(function(e) {
...
})(this, this && this.define); + function(a) {
    "use strict";
...
function generate_left_bubble(elem, clean) {
    ...
    elem.prepend('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + arrow + '" height="' + p_height + '"><polygon points="0,0 ' + p_width + ",0 " + p_width + ",20 " + arrow + ",20 " + (p_width + 15) + ",35 " + (p_width + 9) + ",42 " + (p_width + 3) + ",50 " + (p_width + 1) + ",55 " + p_width + ",60 " + p_width + "," + p_height + " 0," + p_height + '"></polygon></svg>')
}
...
function generate_right_bubble(elem, clean) {
    ...
    var p_height = elem.find("p").height() + 50;
    var arrow = p_width + 30;
    elem.prepend('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + arrow + '" height="' + p_height + '"><polygon points="30,0 30,20 0,20 15,35 21,42 27,50 29,55 30,60 30,' + p_height + " " + arrow + "," + p_height + " " + arrow + ',0"></polygon></svg>')
}
...
function generate_question_bubble(elem, clean) {
    ...
    elem.prepend('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + arrow + '" height="' + p_height + '"><polygon points="30,0 30,20 29,25 27,30 22,37 15,45 0,60 30,60 30,' + p_height + " 85," + p_height + ' 85,0"></polygon></svg>')
}
...
function generate_propose_bubble(elem, clean) {
    ...
    var p_height = elem.find("p").height() + 50;
    var arrow = p_width + 30;
    elem.prepend('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="' + arrow + '" height="' + p_height + '"><polygon points="30,0 30,20 29,25 27,30 22,37 15,45 0,60 30,60 30,' + p_height + " " + arrow + "," + p_height + " " + arrow + ',0"></polygon></svg>')
}
...
function animMenuOverInGame() {
    var checkHover = 0;
    if ($(".selector.dialog-box ul:not(.social-network) a.current").length > 0) {
        ...
        $(".selector.dialog-box ul:not(.social-network) a").hover(function() {
            $mover = $(this).parent().parent().parent().find("div");
            var thisWidth = $(this).outerWidth();
            var thisTopPos = $(this).offset().top - ($(this).parent().parent().parent().offset().top + 3);
            if (checkHover < 1) {
                $mover.css("top", thisTopPos);
                checkHover = checkHover + 1
            }
            if ($(this).parent().parent().parent().hasClass("dropdown-lang")) {
                $speed = "fast"
            } else {
                $speed = "normal"
            }
...
function animPlay() {
    $(".btn-play a").css("width", $(".btn-play a span").width() + 40 + "px")
}

function animGameReport() {
    var nbLines = $(".game-report tbody tr").length;
    if (nbLines > 9) {
        var visibleHeight = 0,
            wholeHeight = 0;
        $(".game-report tbody tr:nth-child(-n+10)").each(function() {
            visibleHeight = visibleHeight + $(this).height()
        });
        $(".game-report tbody tr").each(function() {
            wholeHeight = wholeHeight + $(this).height()
        });
        wholeHeight = wholeHeight + $(".game-report tbody tr").height();
        visibleHeight = visibleHeight + $(".game-report tbody tr").height();
        $(".game-report").css("height", visibleHeight + 10);
        $.clicked = false;
        $(".extend-button").click(function() {
            if ($.clicked) {
                ...
                $(".game-report").css("height", wholeHeight + 40);
                $(this).css("transform", "rotate(-90deg)");
                $.clicked = true
            }
        })
    } else {
        $("div.extend-button").hide()
    }
}

function animCheckboxes() {
    $("body").off("click", ".anim-checkbox");
    $("body").on("click", ".anim-checkbox", function() {
        if ($(this).parent().children("label").hasClass("checked")) {
...
function animActus() {
    ...
    var divNewsBox = $("div.news-box");
    var count = divNewsBox.find("div.actu-slide-content").length;
    if (count > 1) {
        var current = 0;
...
$(window).on("load", function() {
    ...
    $(".dropdown-lang li").click(function() {
        var langClass = $(this).find("span.flag-icon").attr("class");
    ...
    function place_element(target, baseX, baseY, sizeWidth, placeFromLeft) {
        ...
        $("." + target).find("svg").css("width", sizeWidth + "px");
    ...
    var rhombus = '<svg viewBox="0 0 12 12"><path class="rhombus" d="M5.9,1.2L0.7,6.5l5.2,5.4l5.2-5.4L5.9,1.2z" /></svg>';
...
$(window).on("resize", function() {
    if ($(this).width() <= 1738) {
        if ($("div.news-block").hasClass("closed") == false) {
            $("div.news-block").addClass("closed");
            $("div.news-block").find(".close-button").trigger("click")
        }
        if ($("div.last-games").hasClass("closed") == false) {
            $("div.last-games").addClass("closed");
            $("div.last-games").find(".close-games-button").trigger("click")
        }
    }
});
(function($) {
